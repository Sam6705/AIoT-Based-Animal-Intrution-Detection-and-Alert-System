function FINAL_CODES
    ...
    function sendToArduino(code)
        ...
        if toc(lastSendTime) < arduinoCooldown
            return;
        end
    ...
    while ishandle(hFig) && isRunning
        ...
        if ~isempty(boxes)
            ...
            if isAnimal && arduinoConnected
                ...
                if toc(lastSendTime) >= arduinoCooldown || ~strcmp(animalCode, lastSentCode)
                    sendToArduino(animalCode);
                    
                    % Also send specific code based on risk level for animals
                    if riskFactor > 70
                        % High risk animal - send code with 'H' suffix
                        sendToArduino([animalCode 'H']);
                    elseif riskFactor > 40
                        % Medium risk animal - send code with 'M' suffix
                        sendToArduino([animalCode 'M']);
                    end
                end
            end
